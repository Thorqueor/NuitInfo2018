...
public class vipi extends PApplet {
...
public void generateObjectif(){
  ...
  int margin = PApplet.parseInt(random(-10,10));
  if(frameCount%120 + margin == 0){
    if(!ObjectifCreated){
      print("objet cree : "+ str(PApplet.parseInt((frameCount/frameRate))%4) + "secondes"); 
      int x = 960;
      int y = 540 - 115;
...
class Bird{
  ...
  public void update(){
    //print("frame : "+str(frameCount)+" FrameRate : "+str(frameRate)+" calcul : "+str(int(frameCount/frameRate))+" test : "+str(int(frameCount/frameRate)%5)+"\n"); //print de debug de l'infini !!!!!!
    if((PApplet.parseInt((frameCount/frameRate)))%5 == 0){
...
class Droppack {
  ...
  public double droppackHitbox(){
    int area = spriteHeight*spriteWidth;
    return area;
  }

  public void droppackFall(){
    x = (x + kineticFactor);
    y = (y + gravityFactor);
  }
...
class Objectif {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "vipi" };
    if (passedArgs != null) {
